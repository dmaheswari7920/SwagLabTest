...
class RemoteBrowser extends channelOwner_1.ChannelOwner {
    constructor(parent, type, guid, initializer) {
        super(parent, type, guid, initializer);
        this._channel.on('video', ({ context, stream }) => this._onVideo(browserContext_1.BrowserContext.from(context), stream_1.Stream.from(stream)));
    }
    async _onVideo(context, stream) {
        if (!context._videosPathForRemote) {
            stream._channel.close().catch(e => null);
            return;
        }
        const videoFile = path.join(context._videosPathForRemote, utils_1.createGuid() + '.webm');
