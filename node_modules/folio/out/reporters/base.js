...
exports.stripAscii = exports.serializeParameters = exports.formatFailure = exports.BaseReporter = exports.ENV_PREFIX = void 0;
...
class BaseReporter {
    ...
    epilogue(full) {
        ...
        if (expectedFlaky)
            console.log(safe_1.default.yellow(`  ${expectedFlaky} expected flaky`));
        if (unexpectedFlaky.length)
            console.log(safe_1.default.red(`  ${unexpectedFlaky.length} unexpected flaky`));
        if (this.timeout)
...
function formatTestHeader(config, test, index) {
    const tokens = [];
    const spec = test.spec;
    ...
    tokens.push(safe_1.default.bold(safe_1.default.red(pad(header, '='))));
    // Print parameters.
    if (Object.keys(test.parameters).length)
        tokens.push('    ' + ' '.repeat(String(index).length) + safe_1.default.gray(serializeParameters(test.parameters)));
    return tokens.join('\n');
}
